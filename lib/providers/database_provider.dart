...
class DatabaseProvider with ChangeNotifier {
    ...
    await db.transaction((txn) async {
      ...
      for (int i = 0; i < expense_categories.length; i++) {
        await txn.insert(cTable, {
          'title': expense_categories.keys.toList()[i],
          'entries': 0,
  ...
  Future<List<ExpenseCategory>> fetchCategories() async {
    ...
    return await db.transaction((txn) async {
      return await txn.query(cTable).then((data) {
        // 'data' is our fetched value convert it from "Map<String, object>" to "Map<String, dynamic>"
  ...
  Future<void> updateCategory(String category, int nEntries, double nTotalAmount) async {
    final db = await database;
    await db.transaction((txn) async {
      await txn.update( cTable, {  // category table
